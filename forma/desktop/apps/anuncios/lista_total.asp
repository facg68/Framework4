    ...
    <body plantilla="lista" reserva="225">
        ...
        <%
            ...
            if verPantalla <> "*" then
                sqlString = "SELECT DISTINCT Secuencia, Propietario, Titulo, Inicio, Fin, Tipo, VerComo, NombreImagen, Estado, " & _
                                           " NombreUsuario, EscalarImagen, Cuerpo, Segundos, Orden, Codigo, " & _
                            ...
                            "FROM (SELECT a.Secuencia, a.Propietario, a.Titulo, a.Inicio, a.Fin, a.Tipo, a.VerComo, a.NombreImagen, a.EscalarImagen, " & _
                                        " a.Cuerpo, a.Segundos, a.Orden, a.Codigo, aa.Pantalla, u.usuNombre as NombreUsuario, " & _
                                        " CASE WHEN GETDATE() BETWEEN Inicio AND Fin THEN 1 ELSE 0 END AS Estado " & _                            
                                    "FROM dbo.seg_Anuncios AS a INNER JOIN dbo.seg_Anuncios_Asignaciones AS aa ON a.Secuencia = aa.Anuncio " & _
                              "INNER JOIN dbo.seg_Usuarios AS u ON a.Propietario = u.usuCodigo " & _
                                    "WHERE (aa.Pantalla IN (SELECT Pantalla FROM dbo.seg_Anuncios_Pantallas_Usuarios " & _
                                                            "WHERE (Usuario = '" & Request.Cookies("Usuario") & "') " & _
                                                              "AND (Pantalla = '" & verPantalla & "')))) AS anUsuario "
            else
                sqlString = "SELECT DISTINCT Secuencia, Propietario, Titulo, Inicio, Fin, Tipo, VerComo, NombreImagen, Estado, " & _
                                           " NombreUsuario, EscalarImagen, Cuerpo, Segundos, Orden, Codigo, " & _
                                           " dbo.seg_Anuncios_Up_Sec(Propietario, Orden) AS UpSec, dbo.seg_Anuncios_Up(Propietario, Orden) AS UpOrden," & _
                            ...
                            "FROM (SELECT a.Secuencia, a.Propietario, a.Titulo, a.Inicio, a.Fin, a.Tipo, a.VerComo, a.NombreImagen, a.EscalarImagen, " & _
                              ...
                              "INNER JOIN dbo.seg_Usuarios AS u ON a.Propietario = u.usuCodigo " & _
                                    "WHERE (aa.Pantalla IN (SELECT Pantalla FROM dbo.seg_Anuncios_Pantallas_Usuarios " & _
                                                            "WHERE (Usuario = '" & Request.Cookies("Usuario") & "')))) AS anUsuario "
            end if         

            select case estatusAnuncio
                case "a": sqlString = sqlString & "WHERE (Estado = 1) "
                case "h": sqlString = sqlString & "WHERE (Estado = 1) AND (GETDATE() BETWEEN Inicio AND Fin) "                
                case "d": sqlString = sqlString & "WHERE (Estado = 0) "
            end select  
        ...
        <div style="width: 95%; margin: auto;">
                        ...
                        <span style="font-size: 24px">
                            ...
                            <%
                                select case estatusAnuncio
                                    ...
                                    case else: response.write "Publicaciones Totales"
                                end select   
                    ...
                    <td style="text-align:right; width: 60%;">
                        <%
                            ...
                            if not (l.bof or l.eof) then
                                ...
                                %>
                                    <select name="codpantalla" id="codpantalla" required 
                                            style="font-size: 14px; text-align:left; padding: 8px; border-radius: 5px; border: 1px solid rgb(142, 163, 139);"
                                            onChange="filtrar()">
                        ...
                        <select name="verlista" id="verlista" required 
                                style="font-size: 14px; text-align:left; padding: 8px; border-radius: 5px; border: 1px solid rgb(142, 163, 139);"
                                onChange="filtrar()">
            ...
            <table style="width:98%;">
                <tr style="font-size: 14px; background-color: rgb(61, 61, 61); color:rgb(255,255,255);">
                    <td style="padding: 10px; text-align:center; width:  8%;">Estado</td>
                    <td style="padding: 10px; text-align:center; width: 28%;">Titulo</td>
                    <td style="padding: 10px; text-align:center; width: 20%;">Propietario</td>
                    ...
                    <td style="padding: 10px; text-align:center; width: 19%;">&nbsp;</td>
                </tr>
                            ...
                            <table style="width: 100%;">
                                ...
                                %>
                                        <tr style="font-size: 14px; background-color: rgb(255,255,255); color:rgb(0,0,0); border-bottom: 1px solid rgb(194, 194, 194);" >
                                            <td style="padding: 10px; text-align:center; width: 8%; background-color: rgb(<%= bgcolor %>);" onclick="editar(<%= t("secuencia") %>)">
                                                <%
                                            ...
                                            <td style="padding: 5px; text-align:center; width: 28%;" onclick="editar(<%= t("secuencia") %>)">
                                                <%
                                                    response.write t("Titulo")
                                                %>
                                            </td>

                                            <td style="padding: 5px; text-align:center; width: 20%;" onclick="editar(<%= t("secuencia") %>)">
                                                <%
                                                    response.write t("NombreUsuario")
                                                %>
                                            </td>                                            
                                            ...
                                            <%
                                                if (ordenadoPor = 0) and (estatusAnuncio = "a") then
                                                    if isnull(t("DownSec")) then
                                                        ...
                                                        %>
                                                            <td style="padding: 10px; text-align:center; width: 2%;" onclick="intercambiar(<%= t("secuencia") %>, <%= t("orden") %>, <%= t("DownSec") %>, <%= t("DownOrden") %>)">
                                            ...
                                            <td style="padding: 5px; text-align:center; width: 10%;" onclick="editar(<%= t("secuencia") %>)"><%= fechaFormulario(t("Inicio")) %></td>
                                            <td style="padding: 5px; text-align:center; width: 10%;" onclick="editar(<%= t("secuencia") %>)"><%= fechaFormulario(t("Fin")) %></td>
                                            <td style="padding: 5px; text-align:right; width: 19%;">
        ...
        <script>
            function filtrar() {
                ...
                var vinculo ="lista_total.asp?tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;
            ...
            function editar(publicacion) {
                ...
                var vinculo ="editar.asp?w=1&a=" + publicacion + "&tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;
                window.location.href = vinculo;
            }    
            ...
            function intercambiar(secuencia1, orden1, secuencia2, orden2) {
                ...
                var vinculo ="mover.asp?w=1&s1=" + secuencia1 + "&o1=" + orden1 + "&s2=" + secuencia2 + "&o2=" + orden2 + "&tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;
                window.location.href = vinculo; 
            }      
            ...
            function borrar(publicacion) {
                ...
                var estatus = document.getElementById("verlista").value;
                var ordenamiento = document.getElementById("ordenadoPor").value;
                var vinculo ="borrar.asp?w=1&a=" + publicacion + "&tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;                
            ...
            function imagen(publicacion) {
                ...
                var vinculo ="subir_objeto.asp?w=1&a=" + publicacion + "&tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;                                         
                window.location.href = vinculo;          
            }            
            ...
            function ver(publicacion) {
                ...
                var vinculo ="ver.asp?w=1&a=" + publicacion + "&tv=" + pantalla + "&e=" + estatus + "&op=" + ordenamiento;                                         
                window.location.href = vinculo;          
            }     
        </script> 
