    ...
    <head>
        ...
        <%
            ...
            function NombreSistema(Sistema)
                ...
                sqlCommand = "SELECT sysNombre FROM seg_Sistemas WHERE sysCodigo = '" & Sistema & "';"

                set res = cc.execute(sqlCommand)
                    NombreSistema = res("sysNombre")
                res.close: set res = nothing
            end function
        %>    
    </head>

    <body plantilla="lista" reserva="165">
            ...
            <table style="width:100%; margin: auto;">
                <tr style="padding: 10px;">
                    <td colspan ="3" style="text-align:left; width: 20%;">
                        <span style="font-size: 24px">&nbsp;Roles de <%= NombreSistema(Sistema) %></span>
        ...
        <script>
            function filtrar() {
                ...
                var vinculo ="roles_sys.asp?s=<%= Sistema %>&o=" + ordenamiento;
                window.location.href = vinculo;                      
            }
                  
            function editar(codigo) {
                var ordenamiento = document.getElementById("ordenadoPor").value;
                var vinculo ="editar_rol.asp?r=" + codigo + "&o=" + ordenamiento + "&s=<%= Sistema %>";
                window.location.href = vinculo;
            }    

            function borrar(Rol) {
                var confirmacion = confirm("Desea borrar el Rol seleccionada?");
                var vinculo ="borrar_rol.asp?r=" + Rol + "&o=<%= ordenadoPor %>" + "&s=<%= Sistema %>&u=1";                
            ...
            function sUsuarios(codigo) {
                var vinculo ="/forma/desktop/apps/seguridad/usuarios/roles_asignar.asp?r=" + codigo + "&o=1" + "&s=<%= Sistema %>&un=1";               
                window.location.href = vinculo;                
            }     
        </script> 
